                        ...
                        <div key={order.id} className="bg-white border border-gray-200 rounded-lg p-6">
                            <div className="flex justify-between items-start border-b border-gray-100 pb-4 mb-4">
                                ...
                                <div className="text-right">
                                    <span className="inline-block px-3 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-medium">{order.status}</span>
                                    <div className="font-bold text-lg mt-1">${order.totalAmount?.toFixed(2)}</div>
                                </div>
                            </div>
                                ...
                                {order.items.map((item, idx) => (
                                    <div key={idx} className="flex gap-4 items-center">
                                        <div className="w-12 h-12 bg-gray-100 rounded flex items-center justify-center">
                                            <img src={item.image} className="max-w-full max-h-full" alt="thumb"/>
                                        </div>
