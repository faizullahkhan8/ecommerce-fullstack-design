...
/**
 * Enrich product data with mock fields
 * 
 * The API doesn't provide all fields shown in the design,
 * so we add them here with default values from constants.
 * In a real app, these would come from the backend.
 * 
 * @param {Object} product - Base product from API
 * @returns {Object} Enhanced product with all display fields
 */
const enrichProductData = (product) => {
  return {
    ...
    soldCount: PRODUCT_DEFAULTS.SOLD_COUNT,
    
    // Calculate price tiers using utility function
    priceTiers: calculatePriceTiers(product.price),
    
    // Product specifications
    specifications: {
      priceType: PRODUCT_DEFAULTS.PRICE_TYPE,
      type: PRODUCT_DEFAULTS.PRODUCT_TYPE,
      material: PRODUCT_DEFAULTS.MATERIAL,
      design: PRODUCT_DEFAULTS.DESIGN,
      customization: PRODUCT_DEFAULTS.CUSTOMIZATION,
      protection: PRODUCT_DEFAULTS.PROTECTION,
      warranty: PRODUCT_DEFAULTS.WARRANTY
    },
...
const ProductDetailPage = () => {
  ...
  const baseProduct = useSelector(state => state.product.selectedProduct);
  ...
  if (loading || !baseProduct) {
      return (
          <div className="p-20 text-center text-gray-500">
              {LOADING_MESSAGES.PRODUCT}
          </div>
      );
  }

  // Enrich the product data with mock fields
  const product = enrichProductData(baseProduct);
