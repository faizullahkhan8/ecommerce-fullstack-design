import { useDeleteProduct, useGetAllProducts } from "../../api/hooks/product.api.js";
...
const ProductList = () => {
    ...
    const handleDelete = async () => {
        ...
        if (response?.success) {
            setProducts(products.filter((product) => product._id !== productState.data._id));
                ...
                <table className="min-w-full divide-y divide-gray-200 border-separate border-spacing-0">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {getAllProductsLoading ? (
                            <tr className="text-center text-gray-300 font-semibold"><td className="p-4" colSpan={6}>No product added yet!</td></tr>
                        ) : products.length > 0 ? (
                            products.map((product) => (
                                <tr className="hover:bg-blue-50/30 transition-colors group" key={product._id}>
                                    ...
                                    <td className="relative px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        ...
                                        <MoreVertical onClick={() => setActiveMenuId(product._id)} size={24} className="text-primary hover:text-primary/80" />
                                        ...
                                        {activeMenuId === product._id && (
                                            <>
                                                ...
                                                <div className="absolute right-10 top-12 w-32 bg-white rounded-lg shadow-xl border border-gray-100 py-2 z-9999 overflow-hidden">
                                                    ...
                                                    <button
                                                        onClick={() => { setProductState({ type: "delete", data: product }); setActiveMenuId(null) }}
