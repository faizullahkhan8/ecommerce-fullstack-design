...
import { useCreateProuduct, useUpdateProduct } from "../../api/hooks/product.api.js";
...
const AddProduct = () => {
    ...
    const selectedProduct = JSON.parse(searchParams.get("product")) || INITAIL_STATE;
    ...
    useEffect(() => {
        ...
        if (typeof productData.image === 'string') {
            setPreviewUrl(`${import.meta.env.VITE_BACKEND_URL}/${productData.image}`);
        }
    }, [productData.image]);
    ...
    const handleSubmit = async (e) => {
        ...
        if (!isEditing) {
            if (!productData.image) return toast.error("Please select an image");
            ...
            if (response.success) navigate("/admin-dashboard?tab=products-list");
        } else {
            if (productData.image instanceof File) {
                formData.append("image", productData.image);
            }
            ...
            const response = await updateProduct({ product: formData, id: productData._id });
            if (response.success) navigate("/admin-dashboard?tab=products-list");
        }
    };
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 max-w-2xl mx-auto max-md:my-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <div>
                    ...
                    <label htmlFor="image" className="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md hover:border-blue-400 transition-all cursor-pointer overflow-hidden min-h-[200px] items-center">
                        {previewUrl ? (
                            <div className="text-center">
                                <img src={previewUrl} alt="Preview" className="max-h-48 rounded-md mb-2 object-contain" />
                            ...
                            <div className="space-y-1 flex items-center justify-center flex-col">
                                ...
                                <p className="text-sm text-gray-600 font-medium text-blue-600">Upload a file</p>
                                <p className="text-xs text-gray-400">PNG, JPG up to 10MB</p>
                            </div>
                        )}
                        <input type="file" id="image" hidden accept="image/*" onChange={handleChange} />
                    </label>
                </div>

                <div className="pt-4 flex items-center justify-end space-x-3 border-t">
                    <Button type="button" onClick={() => navigate(-1)} variant="outline" className="text-gray-600">Cancel</Button>
                    <Button type="submit" disabled={createProductLoading || updateProductLoading} className="px-8">
                        {(createProductLoading || updateProductLoading) ? <Loader className="animate-spin" /> : (isEditing ? "Update Product" : "Add Product")}
                    </Button>
                </div>
            </form>
        </div>
