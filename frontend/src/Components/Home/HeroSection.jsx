        ...
        <div className="container mx-auto px-4 py-6">
            <div className="bg-white border border-gray-200 rounded-lg p-4 grid grid-cols-1 lg:grid-cols-10 gap-6">
                ...
                <div className="lg:col-span-6">
                    <div className={`relative w-full h-[360px] rounded-md overflow-hidden transition-colors duration-500 ${slides[currentSlide].bg}`}>
                        {/* Banner Content */}
                            ...
                            {slides.map((_, idx) => (
                                <button 
                                    ...
                                    className={`w-2.5 h-2.5 rounded-full transition-all ${
                                        currentSlide === idx ? 'bg-primary w-6' : 'bg-gray-400 hover:bg-gray-600'
                                    }`}
                                />
                            ))}
                        </div>
                ...
                <div className="hidden lg:flex lg:col-span-2 flex-col gap-4">
                    ...
                    <div className="bg-[#e3f0ff] p-4 rounded-md h-[150px] flex flex-col justify-between">
                        ...
                        <div className="space-y-2">
                            <Link to="/register" className="block w-full bg-primary text-white text-center text-sm py-1.5 rounded hover:bg-primary-dark transition-colors">
                                Join now
                            </Link>
                            <Link to="/login" className="block w-full bg-white text-primary text-center text-sm py-1.5 rounded border border-gray-200 hover:bg-gray-50 transition-colors">
                                Log in
                            </Link>
                        </div>
                    </div>
                    ...
                    <div className="bg-[#F38332] p-4 rounded-md text-white flex-1 flex flex-col justify-center">
                        <p className="text-sm opacity-90 mb-1">Get US $10 off</p>
                        <p className="font-medium text-lg leading-tight mb-2">with a new supplier</p>
                    </div>

                    {/* Promo Box 2 - Teal */}
                    <div className="bg-[#55BDC3] p-4 rounded-md text-white flex-1 flex flex-col justify-center">
                        <p className="text-sm opacity-90 mb-1">Send quotes with</p>
                        <p className="font-medium text-lg leading-tight mb-2">supplier preferences</p>
                    </div>
                </div>
            </div>
            ...
            <div className="lg:hidden mt-4">
                 <div className="grid grid-cols-2 gap-2">
                     ...
                     {categories.slice(0, 4).map((cat, idx) => (
                         <Link key={idx} to={`/products`} className="bg-gray-100 p-3 rounded text-sm text-center font-medium">
                             {cat.label}
                         </Link>
                     ))}
                 </div>
